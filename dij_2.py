...
class Dijkstra(object):
    def __init__(self):
        self.window = collections.defaultdict(
    ...
    def dijkstra(self, graph: List[Node], start: Node, end: Node) -> Tuple[int, List[int]]:
        ...
        while pq:
            ...
            if u.idx == end.idx:
                ...
                for i, idx in enumerate(path[::-1], start=1):
                    self.window[idx][i] = True
                    if i >= 1:
                        self.window[idx][i - 1] = True
            ...
            for node_id, w in u.to_ids:
                v = graph[node_id]
                if self.window[v.idx][count] or self.window[v.idx][count + 1]:
                    continue
